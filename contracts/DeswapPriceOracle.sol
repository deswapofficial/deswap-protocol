...
interface IStdReference {
    ...
    /// Returns the price data for the given base/quote pair. Revert if not ayailable.
...
contract DeswapPriceOracle is PriceOracle {
    ...
    function getUnderlyingPrice(DToken dToken) public view returns (uint) {
        if (compareStrings(dToken.symbol(), "dBNB")) {
            ...
            return data.rate;
        }else if (compareStrings(dToken.symbol(), "DAW")) {
