...
/**
 * @title Deswap's DBep20Delegator Contract
...
contract DBep20Delegator is DTokenInterface, DBep20Interface, VDelegatorInterface {
    ...
    function _setImplementation(address implementation_, bool allowResign, bytes memory becomeImplementationData) public {
        require(msg.sender == admin, "DBep20Delegator::_setImplementation: Caller must be admin");
    ...
    function liquidateBorrow(address borrower, uint repayAmount, DTokenInterface dTokenCollateral) external returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("liquidateBorrow(address,uint256,address)", borrower, repayAmount, dTokenCollateral));
        return abi.decode(data, (uint));
    }
   ...
   /**
    ...
    function () external payable {
        require(msg.value == 0,"DBep20Delegator:fallback: cannot send value to fallback");
