...
contract ComptrollerG3 is ComptrollerV3Storage, ComptrollerInterfaceG1, ComptrollerErrorReporter, Exponential {
     ...
     */
    function distributeYAIMinterDeswap(address yaiMinter, bool distributeAll) public {
        if (address(yaiVaultAddress) != address(0)) {
            releaseToVault();
        }
     ...
     */
    function _setDeswapYAIVaultRate(uint deswapYAIVaultRate_) public {
        require(msg.sender == admin, "only admin can");
     ...
     */
    function _setYAIVaultInfo(address vault_, uint256 releaseStartBlock_, uint256 minReleaseAmount_) public {
        require(msg.sender == admin, "only admin can");
