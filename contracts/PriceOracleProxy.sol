...
contract PriceOracleProxy is PriceOracle {
     ...
     * @param vBnbAddress_ The address of dBNB, which will return a constant 1e18, since all prices relative to bnb
    ...
    function getUnderlyingPrice(DToken dToken) public view returns (uint) {
        ...
        if (dTokenAddress == vBnbAddress) {
            // bnb always worth 1
            return 1e18;
        }

        if (dTokenAddress == vUsdcAddress || dTokenAddress == vUsdtAddress) {
            return v1PriceOracle.assetPrices(usdcOracleKey);
        }

        if (dTokenAddress == vDaiAddress) {
            return v1PriceOracle.assetPrices(daiOracleKey);
        }

        if (dTokenAddress == vSaiAddress) {
        ...
        address underlying = DBep20(dTokenAddress).underlying();
